...
log4js.configure({
    appenders:{
        ...
        log_file:{//记录器2：输出到文件
            type : 'file',
            filename: __dirname + `/logs/${programName}_log`,//文件目录，当目录文件或文件夹不存在时，会自动创建
        ...
        data_file:{//：记录器3：输出到日期文件
            type: "dateFile",
            filename: __dirname + `logs/${programName}_data_file`,//您要写入日志文件的路径
...
module.exports = {
    production:log4js.getLogger('production'),
    error:log4js.getLogger('error_log')
}
